...
const waitLabel = seconds => seconds < 60 ? `${seconds}s` : `${Math.floor(seconds / 60)}min ${String(seconds % 60).padStart(2, '0')}s`
</script>
  ...
  <AppLayout>
        ...
        <div class="admin-grid">
          <section class="admin-card users-card" aria-labelledby="users-title">
            <div class="card-heading">
              <div><h2 id="users-title">Usuários <span class="count-badge">{{ users.length }}</span></h2><p>{{ totalActive }} ativos <span aria-hidden="true">·</span> {{ totalAdmins }} {{ totalAdmins === 1 ? 'administrador' : 'administradores' }}</p></div>
              ...
              <li v-for="user in filteredUsers" :key="user.id" class="user-row" :class="{ inactive: !user.ativo }">
                ...
                <div class="user-bottom">
                  ...
                  <div class="user-actions">
                    ...
                    <button type="button" class="icon-button" :class="{ 'deactivate-button': user.ativo }" :disabled="busy || accessDenied || isSelf(user)" :aria-label="`${user.ativo ? 'Desativar' : 'Reativar'} ${user.nome}`" :title="isSelf(user) ? 'Você não pode desativar sua própria conta' : user.ativo ? 'Desativar usuário' : 'Reativar usuário'" @click="askConfirmation(user)">
            ...
            <p v-if="!loading && filteredUsers.length" class="list-caption">{{ filteredUsers.length }} de {{ users.length }} usuários <span>Editar · ativar/desativar</span></p>
          </section>
          ...
          <section class="admin-card create-card" aria-labelledby="create-title">
            ...
            <form class="account-form" @submit.prevent="createAccount">
              <fieldset :disabled="busy || accessDenied">
                <div class="field-pair"><label>Nome completo<input v-model="form.nome" name="nome" autocomplete="off" required maxlength="120" placeholder="Nome do usuário" /></label><label>Apelido<input v-model="form.apelido" name="apelido" autocomplete="off" required maxlength="50" placeholder="Como chamar" /></label></div>
                <div class="field-pair"><label>Telefone<input v-model="form.telefone" name="telefone" type="tel" autocomplete="off" maxlength="25" placeholder="(00) 00000-0000" /></label><label>E-mail<input v-model="form.email" name="email" type="email" autocomplete="off" required maxlength="254" placeholder="usuario@exemplo.com" /></label></div>
              ...
              <button type="submit" class="primary-button create-button" :disabled="busy || accessDenied || createSeconds > 0"><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg>{{ busy && !pendingRegistration ? 'Enviando…' : createSeconds ? `Novo envio em ${waitLabel(createSeconds)}` : 'Criar conta' }}</button>
            </form>
          </section>
        </div>
        <p class="page-note"><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="9" /><path d="M12 11v6m0-10v1" /></svg>Desativar bloqueia o acesso e preserva os dados. No primeiro login, o usuário pode trocar ou manter a senha inicial.</p>
      </div>
    </section>
    ...
    <AppModal :open="!!pendingRegistration" title="Código de verificação" description="Confirme o e-mail para concluir a criação da conta." :busy="busy" @close="closeVerification">
      <form id="verify-account-form" class="modal-form" @submit.prevent="verifyCode">
        ...
        <input id="verification-code" v-model="code" class="code-input" inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" required placeholder="000000" :disabled="busy || accessDenied || verificationBlocked" />
        <p v-if="verificationBlocked" class="field-hint">Este código foi bloqueado após 5 tentativas incorretas. Reenvie para receber um novo código.</p>
        <p v-else-if="expiresLabel" class="field-hint">O código é válido até {{ expiresLabel }}.<span v-if="attemptsRemaining !== null"> {{ attemptsRemaining }} {{ attemptsRemaining === 1 ? 'tentativa restante' : 'tentativas restantes' }}.</span></p>
        <button type="button" class="text-button" :disabled="busy || resendSeconds > 0 || accessDenied" @click="resendCode">{{ resendSeconds ? `Reenviar código em ${waitLabel(resendSeconds)}` : 'Reenviar código' }}</button>
      ...
      <template #footer><button type="button" class="secondary-button" :disabled="busy" @click="closeVerification">Cancelar</button><button type="submit" form="verify-account-form" class="primary-button" :disabled="busy || accessDenied || verificationBlocked">{{ busy ? 'Aguarde…' : 'Verificar e criar conta' }}</button></template>
    </AppModal>
    ...
    <AppModal :open="!!confirmation" :title="confirmationTitle" :busy="busy" @close="closeConfirmation">
      <div v-if="confirmation" class="modal-form">
        <p class="modal-copy"><strong>{{ confirmation.user.nome }}</strong> {{ confirmation.user.ativo ? 'deixará de acessar o VisionFade. Seus dados serão preservados e você poderá reativar a conta depois.' : 'poderá acessar o VisionFade novamente.' }}</p>
        <p v-if="modalError" class="feedback error" role="alert">{{ modalError }}</p>
      </div>
...
.admin-card { min-width: 0; padding: 20px; border: 1px solid #2b3b4c; border-radius: 22px; background: linear-gradient(135deg, #161e2b, #10141df2); }
.card-heading { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 16px; }
...
.users-list { max-height: clamp(180px, calc(100dvh - 505px), 350px); overflow-y: auto; scrollbar-width: thin; scrollbar-color: #455764 transparent; list-style: none; margin: 0; padding: 0; }
...
.verification-note { display: flex; align-items: flex-start; gap: 9px; margin: 12px 0; padding: 10px 12px; border: 1px solid #30495d; border-radius: 12px; background: #182d3e66; }
